...
| `supabase/migrations/20260714_061_frequencia_policies_insert.sql` | ✅ **APLICADA** | 2026-07-14 | **Bug A1b — restaura o conjunto completo de policies de `frequencia` — validada** (5 policies conferidas; **professor salva chamada** ✓). **NÃO reexecutar.** Produção estava com UMA policy só (`frequencia_professor_update`) — sem INSERT ninguém salvava chamada ("new row violates RLS"), sem SELECT ninguém lia. Recriou 5: `frequencia_staff_all` (ALL, staff via user_roles) · `frequencia_select_aluno_propria` (aluno vê a própria) · `frequencia_select_professor_disciplina` (professor da cadeira) · `frequencia_professor_insert` (INSERT endurecido: `professor_id = auth.uid()` + `professor_leciona_disciplina`) · `frequencia_professor_update` (idem no WITH CHECK). DELETE = só staff. Endurecimento vs 031 (INSERT era role-only: qualquer professor lançava em qualquer disciplina). Causa do sumiço: sessão manual no SQL Editor (LICAO-041). Verificação + rollback no arquivo. |
| `supabase/migrations/20260715_062_notas_aluno_select.sql` | 🟡 **CRIADA (aguardando run)** | 2026-07-15 | **Restaura SELECT + DELETE de `notas_aluno`** (produção só com insert+update da 059 → ninguém lia notas: professor não via o que lançou, aluno sem "Minhas Notas", staff nada). SELECT: `aluno_id = auth.uid()` (aluno, Fase C) OU `professor_leciona_disciplina(disciplina_id)` (endurecido vs 031 — antes qualquer professor via TODAS) OU staff via user_roles. DELETE: superadmin-only (forma da 031). INSERT/UPDATE da 059 não tocados. Liga ao aluno por `aluno_id` DIRETO (FK → profiles, 023) — sem passar por matricula. Inclui **levantamento geral** (query pg_policies + conjunto esperado de frequencia/notas_aluno/avaliacoes/matriculas_disciplina/matriculas/profiles) para fechar o "buraco por buraco". Verificação + rollback no arquivo. |
