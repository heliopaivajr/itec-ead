...
| `supabase/migrations/20260627_052_r2_professor_id_drop_convalidacao.sql` | 🟡 **CRIADA (aguardando run)** | — | R2 (G1+G2): `matriculas_disciplina.professor_id` UUID FK→`professores`; **DROP** dos 5 `convalidacao_*` da 051 (redundantes — convalidação canônica = tabela `convalidacoes`). Ainda **não rodada** pelo Hélio; versionada via PR no R2. Drop seguro (colunas nunca referenciadas; `matriculas_disciplina` = 0 linhas / F1). |
