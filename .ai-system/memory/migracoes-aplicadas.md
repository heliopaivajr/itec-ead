...
| `supabase/migrations/20260627_053_recuperacao_status_check.sql` | 🟡 **CRIADA (aguardando run)** | — | R2.2: adiciona `'recuperacao'` ao CHECK de `matriculas_disciplina.status` (aditivo — 6 atuais + recuperacao). Destrava o lançamento retroativo de cadeiras em recuperação (senão 23514). ⚠️ Antes do run, confirmar o nome real do constraint (`pg_constraint` — esperado `matriculas_disciplina_status_check`). |
