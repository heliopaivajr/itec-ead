...
| `supabase/migrations/20260622_049_materiais_disciplina.sql` | 🟡 **CRIADA (aguardando run)** | — | R0.5.4: tabela `materiais_disciplina` + RLS, função `aluno_ve_disciplina()`, bucket privado `materiais-disciplina` + policies. Ainda **não rodada** pelo Hélio. Será versionada via PR na implementação do R0.5.4. |
