...
| R3 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **✅ CONCLUÍDO** — **FRONTEND** (não-RLS). Diagnóstico R3.0 (2026-06-30): RLS de `notas_aluno` **já completa** (023/031, F3 ✅); refinamento professor-por-contrato adiado (F2). Sub-passos: **R3.1 AlunoView ✅** (KPIs reais cursos/freq/média + progresso; hook `useAlunoDashboard`; PR #21) · **R3.2 Painel da Secretaria** — **Leva 1 ✅** (PR #22): `getPendenciasSecretaria` + seção Pendências no AdminView (matrículas a aprovar · docs a validar · taxas · alunos sem vínculo F1); **Leva 2a ✅** (PR #23): funil completo no Matriculas.tsx (abas dos 12 status + InlineStatusSelect) + **aprovação UNIFICADA** (`aprovarMatricula` único caminho: pendente/aguardando_aprovacao → ativa + validado_* + libera acesso; `mudarStatusMatricula` transição livre com efeito de acesso; sair de ativa → role `pendente`) + vocabulário único `constants/statusMatricula.ts` (corrige bug masc/fem do StatusBadge); **Leva 2c ✅** (PR #24): `Alunos.tsx` usa `mudarStatusMatricula` (efeito de acesso em toda tela) + exceção `concluida` mantém acesso (egresso) + `updateStatusMatricula` removida (footgun); **Leva 2b** (pendente, exige migração): checklist de docs obrigatórios persistido · **R3.3 Professores + contratos + ProfessorView** — **R3.3a ✅** (PR #25, decisão C): vínculo = contrato não-encerrado destrava o professor (`getDisciplinasAtivasProfessor` filtra `CONTRATO_ATIVO` em vez de só `assinado`); ProfessoresAdmin gere vínculos (assinar/encerrar soft); ProfessorHome avisa assinatura pendente (formalidade, não bloqueia). **R3.3b ProfessorView data-driven ✅** (PR #26): KPIs reais Disciplinas/Turmas/Alunos/**Notas pendentes** via hook `useProfessorDashboard` (`getDisciplinasAtivasProfessor` + `getResumoDisciplinasProfessor` em `matriculas_disciplina`; notas pendentes = `nota IS NULL`/`cursando`); estado vazio honesto. **→ R3.3 INTEIRO ✅** (vínculo + ProfessorView) · **R3.4 Ficha 360 ✅** (PR #27): header com **Código ITEC**; seção Matrículas com **Nº Matrícula** (`numero_matricula`, só leitura); nova seção **Convalidações** staff-only (`getFichaAluno` estende o loader com `getConvalidacoesByAluno`; leitura + link p/ `/dashboard/convalidacoes`). **→ R3 INTEIRO ✅ CONCLUÍDO** (todos os dashboards por persona + Ficha 360). **Próximo: BLOCO AUDITORIA + QA** (registrado após a linha do R4). Navy #1F3864 / dourado #BF9000. |
| R4 | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| **FINANCEIRO** (trilha E1–E5.1) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **✅ COMPLETO (2026-07-26)** — **migrações 067–078 aplicadas e validadas por query**. E1 acesso Breno (067) · E2a preços (068) · E2b override (069) · E2c geração + seletiva (070/071) · E2c.2 valor inline (072) · 2f confirmação + bucket privado + status automático (073) · 2g Ficha do aluno + dia de vencimento (074) · 2h corrigir sem medo — estorno/cancelamento/guard/auditoria (075) · 2g.1 regenerar cancelado (076) · 2d o aluno vê/paga + PIX + comprovante (077) · config PIX editável · 2e Painel Visão Geral · **E5** travamento 30/90 (078) + telas + **E5.1** destravar fluido/inadimplente visível/cobrança. Fluxo completo testado em produção (suspender→bloqueio→pagar→reativar). **Auth intocado** em todas. Régua de valor única no banco (LICAO-042); billing sagrado (**LICAO-043**). Detalhe no IDEAS-BACKLOG §8. |
...
- 🟠 Restam pré-agosto (report-B, grupo A): **LGPD-01** (professor lê CPF/RG de todos) · **LGPD-02** (exclusão/retenção). Backlog: SEC-02 (upserts mortos em `user_roles`) · ~~`comprovantes-pagamento`~~ **✅ RESOLVIDO (073)** — bucket recriado PRIVADO + signed URL na revisão financeira.
- ✅ **SEC-06** (2026-07-10) — RESOLVIDO pela migração 059 (Camada 0 do módulo Freq/Notas): aluno não insere mais a própria nota; gate real por cadeira/staff + `administracao` lança/corrige nota + professor corrige chamada/justifica falta. Auditoria: 0 notas forjadas. PR `fix/059-policies-sec06`.
- 🏁 **TODOS os bloqueadores de segurança pré-lançamento estão FECHADOS (2026-07-10):** SEC-01 (escalação `user_roles`, 054) · PERF-01 (pré-requisitos) · SEC-03/04/05 (Storage, 055+056) · SEC-06 (aluno-insere-nota, 059). Pendências de segurança restantes são **não-bloqueadoras**: LGPD-01/02 (grupo A), SEC-02 (limpeza), sprint auth (grupo C). [`comprovantes-pagamento` ✅ resolvido pela 073.] Decisão **D-FALTAS**: ✅ definida no §10.
